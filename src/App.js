 ...
 fetch("https://f92c6b35-d121-4c8a-987b-81217155297f.mock.pstmn.io/accounts")
  ...
  componentDidUpdate() {
    this.state.account.forEach(function(d) {
      d.balanceInt = d.balance.replace(/[^0-9.-]+/g,"");  
    });
    
    this.state.transact.forEach(function(d) {
      d.amountInt = d.amount.replace(/[^0-9.-]+/g,"");
  ...
  accPercent() {
    this.state.account.forEach( function(d) {
      d.balanceInt = Math.abs(d.balanceInt);  // alternate function parseFloat(d.balanceInt)
    }); 
  ...
  */
  ...
  showTransactionChart() {
    ...
    svg.selectAll("text.bar")
      ...
      .text( function(d) { return "$" + d.amountInt.toFixed(2) });
